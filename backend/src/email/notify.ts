...
export async function notifyGM(
  surveyId: string,
  playerName: string,
): Promise<void> {
  if (!RESEND_API_KEY) {
    console.warn("RESEND_API_KEY not set — skipping GM notification");
    return;
  }

  const surveyResult = await pool.query(
    "SELECT title, created_by FROM surveys WHERE id = $1",
    [surveyId],
  );
  if (surveyResult.rows.length === 0) return;

  const { title: surveyTitle, created_by: gmId } = surveyResult.rows[0];

  const gmResult = await pool.query(
    "SELECT email FROM gm_profiles WHERE id = $1",
    [gmId],
  );
  if (gmResult.rows.length === 0 || !gmResult.rows[0].email) return;

  const gmEmail: string = gmResult.rows[0].email;
